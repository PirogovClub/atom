    ...
    public void hookSS(Scenario scenario) {
        if (Boolean.parseBoolean(Property.getVariable("extent.pdfReport"))) {
            ...
            try {
                ...
                Reporter.addInfoToStepLog("PDF Updates:");
                if (map != null) {
                    if (map.containsKey("platformName")) {
                        Reporter.addInfoToStepLog("<div id='platform'>Platform Name : " + map.get("platformName").toString() + "</div>");
                        techSuffix = map.get("platformName").toString();
                    }

                    if (map.containsKey("platFormVersion")) {
                        Reporter.addInfoToStepLog("<div id='platFormVersion'>Platform Version : " + map.get("platFormVersion").toString() + "</div>");
                        techSuffix = techSuffix + map.get("platFormVersion").toString();
                    }

                    if (map.containsKey("browser")) {
                        Reporter.addInfoToStepLog("<div id='browser'>Browser : " + map.get("browser").toString() + "</div>");
                        techSuffix = techSuffix + map.get("browser").toString();
                    }

                    if (map.containsKey("version")) {
                        Reporter.addInfoToStepLog("<div id='Version'>Version : " + map.get("version").toString() + "</div>");
                ...
                Reporter.addInfoToStepLog("<div id='pdfReport'>PDF Name : " + pdfFileName + "</div>");
                Reporter.addInfoToStepLog("<div id='sceEndTime' style=\"display:none\">" + new Date() + "</div>");
            } catch (Exception e) {
